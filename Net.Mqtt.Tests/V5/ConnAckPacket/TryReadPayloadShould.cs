...
public class TryReadPayloadShould
{
    ...
    public void ReturnTrue_SetDefaultPropValues_GivenContinuousSequenceWithNoProps()
    {
        ...
        Assert.AreEqual(0x02, packet.StatusCode);
        Assert.IsFalse(packet.SessionPresent);
        Assert.IsTrue(packet.AssignedClientId.IsEmpty);
        Assert.IsTrue(packet.AuthData.IsEmpty);
        Assert.IsTrue(packet.AuthMethod.IsEmpty);
        Assert.IsNull(packet.MaximumPacketSize);
        Assert.AreEqual(QoSLevel.QoS2, packet.MaximumQoS);
        Assert.IsTrue(packet.ReasonString.IsEmpty);
        Assert.AreEqual(ushort.MaxValue, packet.ReceiveMaximum);
        Assert.IsTrue(packet.ResponseInfo.IsEmpty);
        Assert.IsTrue(packet.RetainAvailable);
        Assert.IsNull(packet.ServerKeepAlive);
        Assert.IsTrue(packet.ServerReference.IsEmpty);
        Assert.IsNull(packet.SessionExpiryInterval);
        Assert.IsTrue(packet.SharedSubscriptionAvailable);
        Assert.IsTrue(packet.SubscriptionIdentifiersAvailable);
        Assert.IsTrue(packet.WildcardSubscriptionAvailable);
