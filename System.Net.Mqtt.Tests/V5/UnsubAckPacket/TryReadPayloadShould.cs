...
public class TryReadPayloadShould
{
    ...
    {
        var sequence = new ReadOnlySequence<byte>([0xb0, 0x33, 0x00, 0x02, 0x2d, 0x1f, 0x00, 0x0a, 0x61, 0x6e, 0x79, 0x20, 0x72, 0x65, 0x61, 0x73, 0x6f, 0x6e, 0x26, 0x00, 0x05, 0x70, 0x72, 0x6f, 0x70, 0x31, 0x00, 0x06, 0x76, 0x61, 0x6c, 0x75, 0x65, 0x31, 0x26, 0x00, 0x05, 0x70, 0x72, 0x6f, 0x70, 0x32, 0x00, 0x06, 0x76, 0x61, 0x6c, 0x75, 0x65, 0x32, 0x01, 0x00, 0x02]);

        var actual = Packets.V5.UnsubAckPacket.TryReadPayload(sequence.Slice(2), 51, out var packet);
        ...
        Assert.IsTrue(packet.Properties[0].Name.Span.SequenceEqual("prop1"u8));
        Assert.IsTrue(packet.Properties[0].Value.Span.SequenceEqual("value1"u8));
        Assert.IsTrue(packet.Properties[1].Name.Span.SequenceEqual("prop2"u8));
        Assert.IsTrue(packet.Properties[1].Value.Span.SequenceEqual("value2"u8));
    ...
    public void ReturnTrue_PacketNotNull_GivenValidFragmentedSample()
    {
        var sequence = SequenceFactory.Create<byte>(
            new byte[] { 0xb0, 0x33, 0x00, 0x02, 0x2d, 0x1f, 0x00, 0x0a, 0x61, 0x6e, 0x79 },
    ...
    {
        var sequence = new ReadOnlySequence<byte>([0xb0, 0x33, 0x00, 0x02, 0x2d, 0x1f, 0x00, 0x0a, 0x61, 0x6e, 0x79, 0x20, 0x72, 0x65, 0x61, 0x73, 0x6f, 0x6e, 0x26, 0x00, 0x05, 0x70, 0x72, 0x6f, 0x70, 0x31, 0x00, 0x06, 0x76, 0x61, 0x6c, 0x75, 0x65]);
    ...
    {
        var actual = Packets.V5.UnsubAckPacket.TryReadPayload(ReadOnlySequence<byte>.Empty, 51, out var packet);
