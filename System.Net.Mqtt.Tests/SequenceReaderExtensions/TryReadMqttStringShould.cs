...
public class TryReadMqttStringShould
{
    ...
    {
        var reader = new SequenceReader<byte>(new(new byte[]
        {
    ...
    {
        var start = new MemorySegment<byte>(new byte[]
        {
            0x00, 0x01, 0x02, 0x03, 0x00, 0x01, 0x02, 0x03,
            0x00, 0x13, 0x54, 0x65, 0x73, 0x74, 0x53, 0x74,
            0x72, 0x69, 0x6e, 0x67, 0x2d, 0xd0, 0xa2, 0xd0,
        ...
        var reader = new SequenceReader<byte>(new(start, 0, end, 4));
        reader.Advance(8);
    ...
    {
        var start = new MemorySegment<byte>(new byte[] { 0x00, 0x01, 0x02, 0x03, 0x00, 0x01, 0x02, 0x03 });
    ...
    {
        var start = new MemorySegment<byte>(new byte[] { 0x00, 0x01, 0x02, 0x03, 0x00, 0x01, 0x02, 0x03, 0x00, 0x13 });
        var end = start.Append(new byte[]
            {
    ...
    {
        var start = new MemorySegment<byte>(new byte[] { 0x00, 0x01, 0x02, 0x03, 0x00, 0x01, 0x02, 0x03, 0x00 });
        var end = start.Append(new byte[]
            {
    ...
    {
        var reader = new SequenceReader<byte>(new(Array.Empty<byte>()));
    ...
    {
        var reader = new SequenceReader<byte>(new(new byte[] { 0x00, 0x02 }));
