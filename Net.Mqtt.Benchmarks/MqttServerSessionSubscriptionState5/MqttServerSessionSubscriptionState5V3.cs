...
public sealed class MqttServerSessionSubscriptionState5V3
{
    private volatile FrozenDictionary<byte[], SubscriptionOptions> subscriptions = FrozenDictionary<byte[], SubscriptionOptions>.Empty;
    ...
    public SubscribeResult Subscribe([NotNull] IReadOnlyList<(byte[] Filter, byte Flags)> filters, uint subscriptionId)
    {
        ...
        subscriptions = cloned.ToFrozenDictionary(comparer: ByteSequenceComparer.Instance);
