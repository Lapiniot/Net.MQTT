...
using Net.Mqtt.Extensions;

namespace Net.Mqtt.Benchmarks.Extensions;

public static class SequenceExtensionsV1
{
    ...
    public static bool TryReadMqttHeader(in ReadOnlySequence<byte> sequence, out byte header, out int length, out int offset)
    {
        ...
        if (Mqtt.Extensions.SequenceReaderExtensions.TryReadMqttHeader(ref reader, out header, out length))
        {
